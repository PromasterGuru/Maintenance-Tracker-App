...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            data = jwt.decode(token_required, app.config['SECRET_KEY'])
...
def register_new_user():
    ...
    if username and email and password:
        ...
        if not any(user.get('username', None) == username for user in users.db_users) and not any(
                        mail.get('email', None) == email for mail in users.db_users):
            # the user doesnt exist so try to register them
            try:
                user_container = {}
                user_container['user_id'] = user_id
                ...
                if not re.match(r"(^[a-zA-Z0-9_.]+@[a-zA-Z0-9-]+\.[a-z]+$)", email) and not re.match(
                        r"(^[a-z0-9_.]+@[a-z0-9-]+\.[a-z]+\.[a-z]+$)", email):
                    response = {
...
def login():
    try:
        ...
        if not auth.username or not auth.password:
            ...
            return make_response(jsonify(response)), 401
        if any(user.get('username', None) == auth.username for user in users.db_users) and any(
                        pwd.get('password', None) == auth.password for pwd in users.db_users):
            # generate the access token for auth header
            access_token = jwt.encode(
                {'user_id': users.db_users, 'exp': datetime.datetime.utcnow() + datetime.timedelta(minutes=30)},
...
def delete_requests(request_id):
    rqst = [user_requests for user_requests in rqst if user_requests['request_id'] == request_id]
    rqst.remove(rqst[0])
    response = {'result': rqst}
    return make_response(jsonify(response)), 200
